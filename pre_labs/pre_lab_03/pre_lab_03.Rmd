...
**No, there doesn't appear to be any parsing errors. **
...
**The "amount" and "id" fields are numeric values, but the "name" is a character data type. **
```{r}
...
**There are 135,073 loans for which the forgiveness amount is "N/A"**:
...
**The loans that she "NA" for the forgiveness amount have not been forgiven yet.**:
```{r}
ppp_maryland_loans %>%
...
**There appear to be no loans that were approved in October 2020.**
```{r}
ppp_maryland_loans %>%
...
**The largest loan amount is $10,000,000 and the smallest loan amount is $6. The smallest loan amount is suspicious, because it's an incredibly low number for any business, and I don't see how $6 could help a business in the long-run.**
```{r}
...
**There are two loans that are under $100**
```{r}
...
**The data type of the amount field is  character.**
```{r}
...
**1. The column headers aren't styled consistently. 2. There's an extra row of duplicated data 3. The city "Arnold" is spelled wrong and typed out differently in soem of the rows. **
...
**The column is now called "x1_id"**
...
**The error says "Problem with summarise()column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument. I think the issue is the "amount" column is a character data type, not a numeric value, so the amounts can't be added.  **
...
**There's one duplicate row under Stuart Jones, LLC.**
...
**There are still two misspellings**
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**There is still one misspelling.**
