...
**Answer** Table describing the number of vaccinations administered each day by county in Maryland. 

```{r}
county_vaccinations_by_date <- read.csv("https://raw.githubusercontent.com/state-of-maryland/VaccineCSVs/master/MD_COVID19_TotalVaccinationsCountyFirstandSecondSingleDose.csv") %>%
  clean_names()
...
**Answer** There's one row that has NA in the county column. 
...
**Answer** It might be records of vaccines that are not attributed to any specific county. 
...
**Task** Run the following code and describe the dataframe it produces. Is there a pattern for the most recent 7-10 days? The number of vaccinations per day is relatively lower than before than 10 day timeframe. 
**Answer**
...
**Answer** There's been a steep decline in the number of vaccinations administered daily since this summer. 
...
**Answer** The number of people in Prince George's County fully vaccinated on November 16 decreased by -54.87 percent compared to the week before"
...
**Answer** The chart shows the number of the total Covid-19 cases over time for each MD county. 
...
**Task** Run the following code and describe what the resulting chart shows
**Answer** It appears to be the same data, but it's presented as the rate of increase of covid-19 cases in each county over time. 
...
**Answer** SHows the number of total covid-19 cases in each county. 
...
As it stands, we can run thisj every day and get an up-to-date map.
