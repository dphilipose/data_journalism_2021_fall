...
**Answer** It's table of each state, their Trump-vote percentage, and PPP information (% of businesses receiving loans and the average loan amount per person).
```{r}
...
**Answer** It's a scatter plot that reveals a trend: the higher the percentage of Trump votes in a state, the higher the proportions of small businesses that received PPP loans. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Again, there's a similar trend but the slope of the line isn't as steep, and it's harder to draw any conclusions from the dataset. 
```{r}
...
**Answer** There's a greater correlation when using the percentage of businesses receiving loans than using the average PPP loan amount per employee. 
```{r}
reveal_data %>%
...
**Answer** It says "Pearson's product-moment correlation" and there's variables like "t" and "p-value"
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** Similar output as above, but this output reveals a larger p-value. 
```{r}
...
**Answer** It describes the same variables as the previous dataframe, but now, it's compared between total businesses in each state won by Clinton or Trump, as opposed to a state-by-state comparison. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** It reveals the result of the T-test, and notably, the p-value is relatively high. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Output reveals the results of another T-test, with a lower p-value, indicating that we can be more confident in the results. 
```{r}
