...
wv_ppp_april %>%
  ...
  labs(
    title="West Virginia PPP Loans Approved by Date, April 2020",
    x = "Date",
    y = "Total Loans Approved",
...
state_ppp_loans <- read_csv("data/state_ppp_all.csv") %>%
...
pop_by_state <- get_acs(geography = "state",
              ...
              year = 2019)
clean_pop_by_state <- pop_by_state %>%
  select (-variable, -moe) %>%
  rename(state = NAME, Population = estimate)

state_ppp_and_pop <- state_ppp_loans %>% left_join(clean_pop_by_state, by=c('state')) %>%
  select(-GEOID)
  
state_ppp_per100k <- state_ppp_and_pop %>%  
